	...
	public void show() {
		if (shouldReset) {
			...
			if (sc.hasNextLine()) {
				...
				for (int i = 0; i < splitData.length; i += 3) {
					...
					case 0:
						...
						background.get(i / 3).setPosition(Integer.parseInt(splitData[i + 1]),
								Integer.parseInt(splitData[i + 2]));
						background.get(i / 3).setSize(1800, 1240);
						background.get(i / 3).translate(1800, 0);
						if ((i / 3) % 2 == 0)
							background.get(i / 3).flip(true, false);
						break;
					case 1:
			...
			new Thread(new Runnable() {
				...
				public void run() {
					System.out.println(
							level.getSpawnpoint().getPlayer().getX() + ", " + level.getSpawnpoint().getPlayer().getY());
					c.position.set(level.getSpawnpoint().getPlayer().getX(),
							level.getSpawnpoint().getPlayer().getY() + 200, 0f);
					while (fade > 0f) {
						fade -= .01f;
						c.translate(0, -1f, 0);
						try {
