...
//			this.i.getItems().get(i).setY((int)y);
			...
			if (ammoSelected == i) {
				sp.setColor(InventoryBar.i.getItems().get(ammoSelected).getId() == Item.APPLE
						|| InventoryBar.i.getItems().get(ammoSelected).getId() == Item.SEED
								? new Color(0.1f, 0.75f, 0.1f, 0.9f)
								: new Color(0.75f, 0.1f, 0.1f, 0.9f));
			}
		...
		if (!paused) {
			...
			if (button == Buttons.RIGHT) {
				...
				if (currentlyHeldItem.getId() == Item.NONE) {
					...
					if (currentlyHeldItem.getQuantity() >= 1
							&& (InventoryBar.i.getItems().get(selected).getId() == Item.NONE
